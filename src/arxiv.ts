...
/** Known artifact directories generated by the pipeline — never treat these as source roots. */
...
function effectiveRoot(workDir: string): string {
  ...
  const subdirs = entries.filter((e) => e.isDirectory() && !ARTIFACT_DIRS.has(e.name));
